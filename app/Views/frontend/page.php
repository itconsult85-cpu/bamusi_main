            ...
            <div class="col-lg-10 mx-auto">
                ...
                <?php if (!empty($blocks)): ?>
                    <?php foreach ($blocks as $block): $b = $block['data'] ?? []; $type = $block['block_type'] ?? ''; ?>
                        ...
                        <?php elseif ($type === 'program_list'): ?>
                            <section class="mb-5"><h2 class="h3 fw-bold text-danger mb-4"><?= esc($b['title'] ?? 'Program BAMUSI'); ?></h2><div class="row g-4"><?php foreach (($b['programs'] ?? []) as $program): $programTitle = ($locale === 'en' && !empty($program['name_en'])) ? $program['name_en'] : $program['name']; $programDescription = ($locale === 'en' && !empty($program['description_en'])) ? $program['description_en'] : ($program['description'] ?? ''); $programImage = trim((string)($program['image_url'] ?? '')); if ($programImage && !preg_match('#^https?://#i', $programImage)) $programImage = base_url(ltrim($programImage, '/')); ?><div class="col-md-6 col-lg-4"><a href="<?= base_url('program/' . rawurlencode($program['slug'])); ?>" class="text-decoration-none"><div class="card h-100 border-0 shadow-sm rounded-4 overflow-hidden"><div class="ratio ratio-16x9 bg-danger-subtle"><?php if ($programImage): ?><img src="<?= esc($programImage); ?>" class="w-100 h-100 object-fit-cover" alt="<?= esc($programTitle); ?>"><?php else: ?><div class="d-flex align-items-center justify-content-center text-danger display-5 fw-bold">B</div><?php endif; ?></div><div class="card-body p-4"><small class="text-danger fw-bold text-uppercase"><?= esc($program['division'] ?? 'BAMUSI'); ?></small><h3 class="h5 text-dark fw-bold mt-2"><?= esc($programTitle); ?></h3><p class="text-secondary mb-0"><?= esc(mb_strimwidth(strip_tags($programDescription), 0, 125, '...')); ?></p></div></div></a></div><?php endforeach; ?></div></section>
