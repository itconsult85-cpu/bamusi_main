...
$blockItems = static function (array $data, string $source) use ($sourceRows, $homepageItems): array {
...
<?php foreach ($builderSections as $section): ?>
    ...
    <?php elseif ($type === 'rich_text'): ?><section class="py-5 bg-white"><div class="container py-4"><h2 class="fw-bold text-danger mb-3"><?= esc($data['title'] ?? $t($section, 'title')); ?></h2><article class="page-body fs-5 lh-lg"><?= $data['body'] ?? ''; ?></article></div></section>
