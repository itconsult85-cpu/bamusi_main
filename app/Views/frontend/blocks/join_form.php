            ...
            <div class="col-lg-7 d-flex flex-column justify-content-center p-4 p-lg-5 bg-white">
                <form class="join-interest-form w-100 px-lg-4" novalidate>
                    <div class="row py-3 mb-4" style="row-gap:2rem;column-gap:1rem;">
                        ...
                        <div class="col-12"><label class="d-block text-uppercase fw-bold mb-2 text-danger small" for="join-interest"><?= esc($locale === 'en' ? 'Contribution Interest' : 'Minat Kontribusi'); ?></label><select id="join-interest" name="interest" class="form-select shadow-none px-0 rounded-0 fw-bold" required><option value="" selected disabled><?= esc($locale === 'en' ? 'Choose interest' : 'Pilih minat'); ?></option><?php foreach ($interestItems as $interest): $value = (string) ($interest['item_key'] ?? $interest['title'] ?? $interest['label'] ?? ''); $label = $t($interest, 'label') ?: $t($interest, 'title'); ?><option value="<?= esc($value, 'attr'); ?>" data-label="<?= esc($label, 'attr'); ?>"><?= esc($label); ?></option><?php endforeach; ?><option value="magang" data-label="<?= esc($locale === 'en' ? 'Internship' : 'Magang', 'attr'); ?>"><?= esc($locale === 'en' ? 'Internship' : 'Magang'); ?></option></select></div>
...
<style>.homepage-join-section .form-control:focus,.homepage-join-section .form-select:focus{border-color:#cc0000;box-shadow:0 .25rem .25rem rgba(204,0,0,.12)}.homepage-join-section .form-control,.homepage-join-section .form-select{border:none;border-bottom:1px solid #111;background:transparent}.homepage-join-section .form-control:focus,.homepage-join-section .form-select:focus{border-bottom-color:#cc0000}</style>
