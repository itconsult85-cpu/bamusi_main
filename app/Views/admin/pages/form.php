        ...
        <form action="<?= base_url('admin/pages/save'); ?>" method="post" enctype="multipart/form-data">
            ...
            <div class="card card-primary card-outline mb-4">
                <div class="card-header d-flex justify-content-between align-items-center">
                    ...
                    <div class="d-flex gap-2"><select id="blockType" class="form-select form-select-sm"><option value="rich_text">Teks</option><option value="image">Gambar</option><option value="cards">Kartu manual</option><option value="program_list">Daftar Program otomatis</option><option value="quote">Kutipan</option><option value="cta">Tombol CTA</option><option value="spacer">Jarak</option></select><button type="button" id="addBlock" class="btn btn-sm btn-primary"><i class="fas fa-plus"></i> Tambah</button></div>
                </div>
...
(() => {
    ...
    function fields(block, index) {
        ...
        if (block.type === 'program_list') return `<div class="alert alert-info mb-0"><i class="fas fa-sync-alt me-2"></i>Blok ini otomatis mengambil semua program berstatus terbit dari tabel <code>programs</code>. Perubahan data dilakukan melalui menu <strong>Admin → Program</strong>.</div>`;
